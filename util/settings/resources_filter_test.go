...
func TestResourceInclusionsExclusionNonMutex(t *testing.T) {
	filter := ResourcesFilter{
		ResourceInclusions: []FilteredResource{{APIGroups: []string{"allowlisted-resource"}}},
		ResourceExclusions: []FilteredResource{{APIGroups: []string{"allowlisted-resource"}, Kinds: []string{"blocklisted-kind"}}},
	}

	assert.True(t, filter.IsExcludedResource("allowlisted-resource", "blocklisted-kind", ""))
	assert.False(t, filter.IsExcludedResource("allowlisted-resource", "", ""))
	assert.False(t, filter.IsExcludedResource("allowlisted-resource", "non-blocklisted-kind", ""))
