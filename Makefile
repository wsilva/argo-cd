...
lint-docs:
	...
	find docs -name '*.md' -exec grep -l http {} + | xargs docker run --rm -v $(PWD):/mnt:ro dkhamsing/awesome_bot -t 3 --allow-dupe --allow-redirect --allow-list `cat allow-list | grep -v "#" | tr "\n" ','` --skip-save-results --
